...
class _nroot_NS:
    ...
    # a public feature.
    def __init__(self):
        raise TypeError(u'namespace only, do not instantiate')

    def nth_root(x, n):
        u"""Return the positive nth root of numeric x.
        ...
        if not isinstance(n, int):
            raise TypeError(u'degree n must be an int')
        if n < 2:
            raise ValueError(u'degree n must be 2 or more')
        if isinstance(x, decimal.Decimal):
            return _nroot_NS.decimal_nroot(x, n)
        elif isinstance(x, numbers.Real):
            return _nroot_NS.float_nroot(x, n)
        else:
            raise TypeError(u'expected a number, got %s') % type(x).__name__
    ...
    def nroot(x, n):
        u"""Calculate x**(1/n), then improve the answer."""
    ...
    def iterated_nroot(a, n, g):
        u"""Return the nth root of a, starting with guess g.
    ...
    def bignum_nroot(x, n):
        u"""Return the nth root of a positive huge number."""
        assert x > 0
...
def geometric_mean(data):
    u"""Return the geometric mean of data.
    ...
    if iter(data) is data:
        data = list(data)
    errmsg = u'geometric mean does not support negative values'
    n = len(data)
    if n < 1:
        raise StatisticsError(u'geometric_mean requires at least one data point')
    elif n == 1:
        x = data[0]
